...
Line new_line_car(LineType type, int y, LineDirection direction, int speed_limit, int next_car_in, int min_next_car, int max_next_car, float stopper_chance_friendly) {
    Line line = {type, y};
    Timer timer;
    timer_start(&timer);
    line.line_data.car.spawn_timer = timer;
    ...
    if (stopper_chance_friendly < -0.5) {
        line.line_data.car.stopper_chance = 0;
        line.line_data.car.friendly_lane = 1;
    } else {
        line.line_data.car.stopper_chance = stopper_chance_friendly;
    }
    return line;
}
